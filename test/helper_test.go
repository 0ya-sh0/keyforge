...
// runApp starts and stops the server for each test
func runApp(t *testing.T) (*exec.Cmd, func()) {
	...
	// Wait for the server to start with 10 retries using the health endpoint
	for i := 0; i < 10; i++ {
		// Call the health endpoint to check if the server is running
...
// getGrpcConnection returns a new gRPC connection to a local instance of the server
